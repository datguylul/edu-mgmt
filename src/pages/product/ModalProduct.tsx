...
const ModalEditStaffInfo: React.FC<ModalProp> = ({ showModal = false, productId = '', onCloseModal }) => {
  ...
  return (
    <Form {...formItemLayout} form={form} name="register" onFinish={handleUpdateProduct} scrollToFirstError>
      ...
      <Form.Item name="ImportPrice" label="Giá Nhập" rules={[{ required: true, message: 'Giá nhập không thể trống!' }]}>
        <Input />
      </Form.Item>
      ...
      {productId && (
        <React.Fragment>
          <Form.Item name="CreateDate" label="Ngày tạo" preserve={true}>
            <Input disabled={true} />
          </Form.Item>
          <Form.Item name="UpdateDate" label="Thời gian sửa" preserve={true}>
            <Input disabled={true} />
          </Form.Item>
        </React.Fragment>
      )}
